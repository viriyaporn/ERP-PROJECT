...
export default function CreatePackageServiceForm({
  onSubmit,
  ...
  return (
    <form onSubmit={handleSubmit(data => onSubmit(data, reset))}>
      <div className="flex flex-col mb-5 ">
        <label htmlFor="text" className="mb-2 ">บริการเสริมของบรรจุภัณฑ์ (ไทย)</label>
        <Controller
          name="nameTh"
          control={control}
          rules={{ required: true }}
          render={({ field: { onChange, value } }) => (
            <Input type="text" onChange={onChange} value={value} placeholder="ระบุชื่อบริการพิเศษ (ไทย)" />
          )}
        />
        {errors.nameTh && <span className='text-red-500'>nameTh ไม่ถูกต้อง</span>}
      </div>
      <div className="flex flex-col mb-5 ">
        <label htmlFor="text" className="mb-2 ">บริการเสริมของบรรจุภัณฑ์ (อังกฤษ)</label>
        <Controller
          name="nameEn"
          control={control}
          rules={{ required: true }}
          render={({ field: { onChange, value } }) => (
            <Input type="text" onChange={onChange} value={value} placeholder="ระบุชื่อบริการพิเศษ (อังกฤษ)" />
          )}
        />
