    ...
    { field: 'Note', headerName: 'หมายเหตุ', filter: true, width: 140 },
    {
      ...
      cellRenderer: info => (
        <div className="flex space-x-1 self-center text-center justify-center align-center">
          <button type="button" onClick={() => onBtnClick(info)} className="rounded w-[170px]  justify-center self-center text-center bg-yellow-500 hover:text-blue-500">กำลังอยู่ระหว่างการฆ่าเชื้อ</button>
  ...
  const onBtnClick = async info => {
    try {
      const swal = await Swal.fire({
        title: 'ฆ่าเชื้อเสร็จแล้วใช่หรือไม่',
        // text: "คุณต้องการยืนยันการรับอุปกรณ์ใช่หรือไม่",
        icon: 'warning',
      ...
      Swal.fire('สำเร็จ', 'ยืนยันการฆ่าเชื้อเรียบร้อย', 'success');

      // refetchProducts();
    } catch (err) {
      console.error('onSubmitCreateForm ERROR:', err);
      return Swal.fire('ขออภัย', err?.message || "ไม่สามารถยืนยันการฆ่าเชื้อได้", 'error');
    }
  }
    ...
    <>
      <LayoutOffice>
        <div className="flex justify-between p-2">
          <h1 className="font-bold text-3xl mb-5">อุปกรณ์การแพทย์อยู่ในกระบวนการฆ่าเชื้อ</h1>
          <div>
            {/* <button
              type="button"
