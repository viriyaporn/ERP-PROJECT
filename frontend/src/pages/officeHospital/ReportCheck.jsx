    ...
    { field: 'Note', headerName: 'หมายเหตุ', filter: true, width: 140 },
    {
      ...
      cellRenderer: info => (
        <div className="flex space-x-1 self-center text-center justify-center align-center">
          <button type="button" onClick={() => onBtnClick(info)} className="rounded w-[100px]  justify-center self-center text-center bg-yellow-500 hover:text-blue-500">รอตรวจสอบ</button>
  ...
  const onBtnClick = async info => {
    try {
      const swal = await Swal.fire({
        title: 'คุณต้องการยืนยันการรับอุปกรณ์ใช่หรือไม่',
      ...
      Swal.fire('สำเร็จ', 'ยืนยันการรับอุปกรณ์เรียบร้อย', 'success');

      // refetchProducts();
    } catch (err) {
      console.error('onSubmitCreateForm ERROR:', err);
      return Swal.fire('ขออภัย', err?.message || "ไม่สามารถยืนยันการรับอุปกรณ์ได้", 'error');
    }
  }
    ...
    <>
      <LayoutOffice>
        <div className="flex justify-between p-2">
          <h1 className="font-bold text-3xl mb-5">อุปกรณ์การแพทย์ที่รอการตรวจสอบ</h1>
          <div>
            {/* <button
              type="button"
